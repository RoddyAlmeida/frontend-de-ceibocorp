...
export function buildReceiptHtmlString(
  venta: ReciboVenta,
  sellerName: string,
  sedeName?: string,
  showSede?: boolean,
  ...
  const rows = details
    .map(
      (d) =>
        `<tr><td>${escapeHtml(nombreProductoVenta(d))}</td><td style="text-align:right">${parseIntSafe(d.quantity)}</td><td style="text-align:right">$${parseNum(d.price).toFixed(2)}</td><td style="text-align:right;font-weight:700">$${(parseNum(d.price) * parseIntSafe(d.quantity)).toFixed(2)}</td></tr>`,
    )
    .join('');

  const infoRows = [
    `<div class="row"><span>N.° de nota:</span><span>${venta.id ?? '—'}</span></div>`,
    `<div class="row"><span>Fecha:</span><span>${formatFechaVenta(venta.created_at ?? null)}</span></div>`,
  ];

  if (sellerName) {
    infoRows.push(`<div class="row"><span>Vendedor:</span><span>${escapeHtml(sellerName)}</span></div>`);
  }

  infoRows.push(
    `<div class="row"><span>Cliente:</span><span>${escapeHtml(venta.customer_name ?? 'Consumidor Final')}</span></div>`,
  );
  ...
  <div style="text-align:center">
    ...
    ${showSede && sede ? `<div style="font-size:11px;color:#4b5563">${escapeHtml(sede)}</div>` : ''}
  ...
  <hr/>
  <div class="grid">${infoRows.join('')}</div>
  <hr/>
